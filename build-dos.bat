...
@REM docker build -t %DOCKER_IMAGE% -f %PROJECT_ROOT%/Dockerfile.dos .

@set REMOTE_PROJECT_ROOT=/%PROJECT_ROOT::=%
set REMOTE_PROJECT_ROOT=%REMOTE_PROJECT_ROOT:\=/%

( docker run -i --rm -v %REMOTE_PROJECT_ROOT%:/project %DOCKER_IMAGE% stdbuf -oL -eL /project/in_docker_build.sh Makefile %* && touch linux_build_succeeded.tmp )
